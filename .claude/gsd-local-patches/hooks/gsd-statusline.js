...
function runStatusline() {
  ...
  try {
    ...
    let cavemanBadge = "";
    try {
        const cavemanFlagPath = path.join(homeDir, '.claude', '.caveman-active');
        if (fs.existsSync(cavemanFlagPath)) {
            const mode = fs.readFileSync(cavemanFlagPath, 'utf8').trim();
            if (mode === "full" || !mode) {
                cavemanBadge = "\x1b[38;5;172m[CAVEMAN]\x1b[0m ";
            } else if (mode !== "off") {
                cavemanBadge = `\x1b[38;5;172m[CAVEMAN:${mode.toUpperCase()}]\x1b[0m `;
            }
        }
    } catch (e) {
        // Fail silently if Caveman file can't be read
    }
    // ---------------------------
    ...
    if (middle) {
      process.stdout.write(`${cavemanBadge}${gsdUpdate}\x1b[2m${model}\x1b[0m │ ${middle} │ \x1b[2m${dirname}\x1b[0m${ctx}`);
    } else {
      process.stdout.write(`${cavemanBadge}${gsdUpdate}\x1b[2m${model}\x1b[0m │ \x1b[2m${dirname}\x1b[0m${ctx}`);
    }
  } catch (e) {
